...
function getVaryParamsTargetForWorkUnit(
  ...
  if (process.env.__NEXT_LEDGERS) {
    return target as SetLedger<VaryParamId>
  }
...
export function createVaryParamsAccumulator(): SetLedger<VaryParamId> | null {
  return getVaryParamsTarget('segment')
}

export function getMetadataVaryParamsAccumulator(): SetLedger<VaryParamId> | null {
 ...
 */
export function getRootParamsVaryParamsAccumulator(): AsyncIterable<VaryParamId> | null {
...
export function createVaryingParams(
  accumulator: SetLedger<VaryParamId>,
...
export function createVaryingSearchParams(
  ...
  return new Proxy(originalSearchParamsObject, {
    get(target, prop, receiver) {
      if (typeof prop === 'string') {
        addToLedger(accumulator, SEARCH_PARAMS_VARY_ID)
      }
      return Reflect.get(target, prop, receiver)
    },
    has(target, prop) {
