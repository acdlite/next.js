...
export function completeSoftNavigation(
  ...
  // minimum information needed for the server to intercept the route.
  const changedPath = computeChangedPath(oldState.tree, tree)
  const nextUrlForNewRoute = changedPath ? changedPath : referringNextUrl
  ...
  const previousNextUrl =
    // If the current route was not intercepted, then we can omit this
    // from subsequent refreshes, i.e. set this to null.
    // TODO: Get rid of this extra traversal by computing this during the
    // same traversal that computes the tree itself.
    nextUrlForNewRoute !== null && hasInterceptionRouteInCurrentTree(tree)
      ? nextUrlForNewRoute
      : null
    ...
    focusAndScrollRef: {
      ...
      hashFragment:
        ...
        shouldScroll && url.hash !== ''
          ...
          : null,
      segmentPaths:
        // During a hash-only hange, setting scrollableSegmeths to an empty
