      ...
      switch (fetchStrategy) {
        case FetchStrategy.PPR: {
          ...
          if (
            ...
            (walkRequiresRuntimeCompleteness(staticWalkStrategy, route) &&
              (head.prefetchHints &
          ...
          // point in pingSegmentInCacheComponentsTree. Which segments actually
          // need a runtime request — registered directly, or only as the
          // fallback after an insufficient static attempt — was decided there.
              ...
              if (spawnedEntries.size > 0) {
                spawnPrefetchSubtask(
                  fetchSegmentPrefetchesUsingRuntimeRequest(
                    ...
                    route,
                    runtimeStrategy,
                    // When the head is the only pending entry, the server is
                    // told to skip the segments.
                    spawnedEntries.size === 1 &&
                      spawnedEntries.has(head.requestKey)
                      ? null
                      : requestTree,
                    spawnedEntries
                  )
          ...
          if (needsDynamicRequest) {
            spawnPrefetchSubtask(
              fetchSegmentPrefetchesUsingRuntimeRequest(
                ...
                route,
                fetchStrategy,
                // When the head is the only pending entry, the server is told
                // to skip the segments.
                spawnedEntries.size === 1 && spawnedEntries.has(head.requestKey)
                  ? null
                  : dynamicRequestTree,
                spawnedEntries
              )
 ...
 * pingSegmentInCacheComponentsTree.
 */
 ...
 * whether any of the node's param values changed — if so, the navigation
 * replaces the node's data, and the node begins the new part of the route
 * too. A node the navigation keeps is prefetched at the ordinary static tier.
...
function pingSharedPartOfCacheComponentsTree(
  ...
  if (
    ...
    // The navigation replaces this segment's data, and the data of everything
    // below it (see updateRenderTreeOnNavigation in render-tree.ts).
    return pingNewPartOfCacheComponentsTree(
      now,
      task,
      route,
 ...
 * can complete (see walkRequiresRuntimeCompleteness) — this is where the
 * segment decides between a static attempt and a runtime deopt. If the
 * segment's node carries the ShouldAttemptStaticPrefetch hint (the
 * build-time prerender accessed no runtime data), its subtree is prefetched
 * statically first, and the responses themselves decide whether that was
 * enough: every fulfilled entry carries a needsRuntimeRequest signal. Pending
 * responses block the task, so the attempt is serial, never raced: static
 * attempt → observe → runtime only if needed. Without the hint, the segment
 * deopts directly. Deopting registers the segment's request key in
 * spawnedRuntimePrefetches; the runtime gate at the end of pingRootRouteTree
 * issues a single batched runtime request for everything that accumulated,
 * and that request re-fetches the whole subtree.
 *
